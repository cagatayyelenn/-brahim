...
if (!function_exists('fmtTL')) {
    function fmtTL($n){ return number_format((float)$n, 2, ',', '.').' TL'; }
}
if (!function_exists('trDate')) {
    function trDate($d){ return date('d.m.Y', strtotime($d)); }
}
?>
...
<head>
    ...
    <style>
        ...
        .receipt {
            ...
            padding: 15px;
            height: 135mm; /* Sayfanın yarısı eksi biraz boşluk */
            display: flex;
        ...
        .details strong {
            display: inline-block;
            width: 100px; /* Sabit genişlik ile hizalama sağlar */
        }
        .payment-text {
            margin-bottom: 15px;
            line-height: 1.5;
        ...
        <div class="receipt">
            ...
            <div class="details">
                <div class="details-left">
                    <div><strong>EĞİTİM DÖNEMİ</strong>: <?= htmlspecialchars($egitimdonemi ?? (date('Y').'-'.(date('Y')+1))) ?></div>
                ...
                <div class="details-right">
                    ...
                    <div><strong>MAKBUZ NO</strong>: <?= date('dmy') . '-T' . $taksit_id . 'S' . $sozlesme_id;  ?></div>
                    <div><strong>ÖDEME TUTARI</strong>: <?= fmtTL($odemetutari) ?></div>
                </div>
            </div>

            <div class="payment-text">
                Sn. <?= htmlspecialchars($ogrenciadsoyad) ?>’dan yukarıda bilgileri yazılı olan sözleşme taksitine istinaden
                <?= htmlspecialchars(date('d.m.Y'))?> tarihinde <?= htmlspecialchars(mb_strtoupper($odemeturu)) ?> ile
            ...
            <div class="summary-text">
                <?= htmlspecialchars($sozlesmeno) ?> numaralı sözleşme için <?= htmlspecialchars(date('d.m.Y')) ?> tarihinde
                <?= date('dmy') . '-T' . $taksit_id . 'S' . $sozlesme_id;  ?> belge numarasıyla yapılan <?= fmtTL($odemetutari) ?> (<?= htmlspecialchars(mb_strtoupper($odemeturu)) ?>)
            ...
            <div class="footer">
                ...
                <div class="signature">
                    ...
                    <div class="signature-name"><?= htmlspecialchars($odemeyialan ?: ($_SESSION['kisi_adi'] ?? 'NGLS Personeli')) ?></div>
                    <div>İmza / Kaşe</div>
                </div>
            </div>
        </div>
