                        ...
                        <form id="contractForm" method="POST" action="">
                            ...
                            <div id="step4" class="step-section" style="display:none;">
                                <h5 class="mb-3 text-primary d-flex justify-content-between align-items-center">
                                    <span>4. Taksit Bilgisi</span>
                                    <div class="form-check form-switch ms-auto">
                                        <input class="form-check-input" type="checkbox" id="chkManuelTaksit">
                                        <label class="form-check-label" for="chkManuelTaksit">Manuel Oluştur</label>
                                    </div>
                                </h5>
                                ...
                                <div class="row">
                                    ...
                                    <!-- Otomatik Mod Kontrolleri -->
                                    <div class="col-md-8 mb-3 auto-mode-controls">
                                        <div class="row">
                                            <div class="col-md-6">
                                    ...
                                    <div class="col-md-8 mb-3 manual-mode-controls" style="display:none;">
                                        <div class="text-end h-100 d-flex align-items-end justify-content-end">
                                            <button type="button" class="btn btn-primary" onclick="manuelSatirEkle()">
                                                <i class="ti ti-plus"></i> Taksit Ekle
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <table class="table table-bordered table-sm" id="taksitTablosu">
                                        <thead class="bg-light">
                                            <tr>
                                                ...
                                                <th>Vade Tarihi</th>
                                                <th>Tutar</th>
                                                <th style="width: 50px;" class="text-center">Sil</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <!-- JS ile dolacak -->
...
<script>
    ...
    // Event Listeners
    document.getElementById('chkManuelTaksit').addEventListener('change', function(){
        var isManual = this.checked;
        if(isManual){
            $('.auto-mode-controls').hide();
            $('.manual-mode-controls').show();
            // Tabloyu temizle, manuel girişe hazırla
             document.querySelector('#taksitTablosu tbody').innerHTML = '';
             manuelSatirEkle(); // İlk satırı ekle
        } else {
            $('.manual-mode-controls').hide();
            $('.auto-mode-controls').show();
             document.querySelector('#taksitTablosu tbody').innerHTML = ''; // Temizle
        }
    });
    ...
    function taksitOlustur() {
        ...
        for (var i = 0; i < taksitSayisi; i++) {
            ...
            var tarihFormat = yil + '-' + ay + '-' + gun; // Input date formatı YYYY-MM-DD

            // Kuruş farkını son taksite ekle
            ...
            // Otomatik modda inputlar readonly olabilir veya olmayabilir. 
            // Kullanıcının değiştirmesine izin verelim ama "Otomatik" modda genelde hesaplanan kalır.
            // Ama veri toplarken inputlardan okuyacağız.
            
            var row = `<tr>
                <td>${i + 1}</td>
                <td><input type="date" class="form-control form-control-sm taksit-date" value="${tarihFormat}"></td>
                <td><input type="text" class="form-control form-control-sm taksit-tutar money-input" value="${formatMoney(buTaksit)}"></td>
                 <td class="text-center">
                    <button type="button" class="btn btn-sm btn-light text-muted" disabled><i class="ti ti-trash"></i></button>
                </td>
            </tr>`;
            tbody.innerHTML += row;
        }
        
        // Yeni eklenen money-input'lar için maskelemeyi tekrar bağla
        rebindMoneyInput();
    }
    ...
    function manuelSatirEkle() {
        ...
        var row = `<tr>
            <td>${rowCount}</td>
             <td><input type="date" class="form-control form-control-sm taksit-date" value="${today}"></td>
            <td><input type="text" class="form-control form-control-sm taksit-tutar money-input" placeholder="0,00"></td>
            <td class="text-center">
    ...
    $(document).on('click', '.remove-row', function(){
        ...
        $('#taksitTablosu tbody tr').each(function(index){
            $(this).find('td:first').text(index + 1);
        });
    });

    function rebindMoneyInput() {
        $('.money-input').off('blur focus').on('blur', function () {
            var val = parseMoney($(this).val());
            $(this).val(formatMoney(val));
        }).on('focus', function () {
            var val = parseMoney($(this).val());
            if (val === 0) $(this).val('');
            else $(this).val(val.toString().replace('.', ','));
        });
    }
</script>
<script>
    ...
    $('#contractForm').on('submit', function (e) {
        ...
        $('#taksitTablosu tbody tr').each(function () {
            var tarihVal = $(this).find('.taksit-date').val(); // YYYY-MM-DD
            var tutarVal = $(this).find('.taksit-tutar').val(); // 1.250,00

            formData.taksitler.push({
                tarih: tarihVal,
                tutar: parseMoney(tutarVal)
            });
        });
