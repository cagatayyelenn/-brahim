...
foreach ($rows as $r) {
    ...
    if ($r['taksit_id']) {
        ...
        if ($paidFull) {
            ...
            if ($late) $contracts[$sid]['ozet']['gecikmis_adet']++;
            else       $contracts[$sid]['ozet']['kalan_adet']++;
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <ul class="nav nav-tabs nav-tabs-bottom mb-4">
                ...
                <li><a href="ogrenci-detay-sozlesme.php?id=<?= htmlspecialchars($ogr_no) ?>" class="nav-link active"><i class="ti ti-bookmark-edit me-2"></i>Sözleşme Ve Taksitler</a></li>
                    ...
                    <div class="accordions-items-seperate" id="accordionSoz">
                        <?php if (empty($contracts)): ?>
                            <div class="alert alert-warning d-flex align-items-center">
                                ...
                                <a class="btn btn-primary btn-sm ms-auto" href="sozlesme-olustur.php?id=<?= (int)$ogr_no ?>">Sözleşme Oluştur</a>
                            </div>
                        <?php else: ?>
                            <?php $i=0; foreach ($contracts as $sid => $c):
                                $h = $c['header']; $o = $c['ozet']; $taksitler = $c['taksitler'];
                                $collapseId = "sz_".$sid;
                                    ...
                                    <h2 class="accordion-header d-flex align-items-center">
                                        <button class="accordion-button flex-grow-1 <?= $i===0 ? '' : 'collapsed' ?>"
                                            ...
                                            <?php if (!empty($h['sozlesme_tarihi'])): ?>
                                                <span class="ms-2 text-muted">(Tarih: <?= date('d.m.Y', strtotime($h['sozlesme_tarihi'])) ?>)</span>
                                            <?php endif; ?>
                                        </button>
                                        <?php
                                        $odemeVar = false;
                                        ...
                                        ?>
                                        <a href="sozlesme-guncelleme.php?id=<?= (int)$sid ?>" class="btn btn-outline-primary btn-sm  flex-shrink-0"   >
                                            <i class="ti ti-file-text"></i> Düzenle
                                        </a>

                                        <a href="sozlesme-belge.php?id=<?= (int)$sid ?>" class="btn btn-outline-primary btn-sm  flex-shrink-0" style="margin-right: 16px;" target="_blank" rel="noopener">
                                            <i class="ti ti-file-text"></i> Sözleşme Belgesi
                                        </a>
                                    </h2>

                                    <div id="<?= $collapseId ?>" class="accordion-collapse collapse <?= $i===0 ? 'show' : '' ?>" data-bs-parent="#accordionSoz">
                                                ...
                                                <table class="table">
                                                    ...
                                                    <?php if ($taksitler): ?>
                                                        <?php foreach ($taksitler as $idx => $t):
                                                            ...
                                                            <tr>
                                                                <td><?= $idx+1 ?></td>
                                                                <td><?= htmlspecialchars(date('d.m.Y', strtotime($t['vade_tarihi']))) ?></td>
                                                                <td><?= number_format((float)$t['tutar'], 2, ',', '.') ?> ₺</td>
                                                                <td><?= number_format((float)$t['odenen'], 2, ',', '.') ?> ₺</td>
                                                                <td> <span class="badge <?= $badge ?> d-inline-flex align-items-center"> <i class="ti ti-circle-filled fs-5 me-1"></i><?= $label ?> </span>
                                                                    <?php if (!$paid && $due > 0): ?>
                                                                        <small class="text-danger ms-1">Eksik: <?= number_format($due, 2, ',', '.') ?> ₺</small>
                                                                    <?php endif; ?>
                                                                </td>
                                                                <td class="text-end">
                                                                    <?php if ($paid): ?>
                                                                        <a class="btn btn-light btn-sm" href="odeme-belge.php?taksit=<?= (int)$t['taksit_id'] ?>" target="_blank">Belge Göster</a>
                                                                    <?php else: ?>
                                                        ...
                                                        <?php endforeach; ?>
                                                    <?php else: ?>
                                                        <tr><td colspan="6" class="text-muted text-center">Bu sözleşmeye ait taksit bulunamadı.</td></tr>
                                                    <?php endif; ?>
                                                    </tbody>
                                                    ...
                                                    <tr>
                                                        <td colspan="6" class="bg-dark text-white">
                                                            <div class="d-flex flex-wrap align-items-center justify-content-between">
                                                                <div>Toplam Sözleşme: <strong><?= number_format($toplamSozlesme,2,',','.') ?> ₺</strong><span class="mx-2">|</span></div>
                                                                <div>Ödenen: <strong><?= number_format($odenenTutar,2,',','.') ?> ₺</strong><span class="mx-2">|</span></div>
                                                                <div>Ödenmesi Gereken: <strong><?= number_format($odenmesiGereken,2,',','.') ?> ₺</strong><span class="mx-2">|</span></div>
                                                                <div>Gecikmiş Tutar: <strong class="text-warning"><?= number_format($o['gecikmis_tutar'],2,',','.') ?> ₺</strong></div>
                                                                <div class="ms-auto">
                                                                    <span class="badge bg-secondary me-1">Taksit: <?= (int)$o['taksit_say'] ?></span>
                                                                    <span class="badge bg-success me-1">Ödenen: <?= (int)$o['odenen_adet'] ?></span>
                                                                    <span class="badge bg-warning me-1">Kalan: <?= (int)$o['kalan_adet'] ?></span>
                                                                    <span class="badge bg-danger">Gecikmiş: <?= (int)$o['gecikmis_adet'] ?></span>
                                                                </div>
                                                            </div>
                                                        </td>
                                                    </tr>
                            ...
                            <div class="row">
                                <div class="col-lg-4">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control text-warning text-end" id="ozetToplam" value="50.000,00 TL" readonly>
                                    </div>
                                </div>
                                <div class="col-lg-4">
                                    <div class="mb-3">
                                        <label class="form-label text-success text-muted mb-1">Alınan Peşinat</label>
                                        <input type="text" class="form-control text-success text-end" id="ozetPesinat" value="20.000,00 TL" readonly>
                                    </div>
                                </div>
                                <div class="col-lg-4">
                                    <div class="mb-3">
                                        <label class="form-label text-danger mb-1">Kalan Borç Tutarı</label>
                                        <input type="text" class="form-control text-end text-danger fw-semibold" id="ozetKalan" value="30.000,00 TL" readonly>
                                    </div>
                                </div>
                                <div class="col-lg-4">
                                    <div class="mb-3">
                                        <label class="form-label text-info mb-1">Taksit Tutarı</label>
                                        <input type="text" class="form-control text-info text-end" id="taksitTutarGoster" value="3.000,00 TL" readonly>
                                    </div>
                                </div>
                                <div class="col-lg-4">
                                    <div class="mb-3">
                                    ...
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-lg-12">
                                            <div class="mb-0">
                                                ...
                                                <input type="text" class="form-control text-end" id="inpOdenecekTutar" placeholder="0,00 TL" disabled>
                                            </div>
                                        </div>
                                    </div>
                                </div>
