...
try {
    ...
    if (!$kasa_id || !$hareket_tur_id || $tutar <= 0) {
        echo json_encode(['ok' => false, 'msg' => 'Eksik/hatalı veri (kasa, tür, tutar)']);
        exit;
    }
    ...
    if (!$insertID) {
        echo json_encode(['ok' => false, 'msg' => 'Kayıt eklenemedi']);
        exit;
    }
    ...
    // Loglama
    $logAciklama = 'Kasa gideri eklendi: ' . number_format((float) $tutar, 2, ',', '.') . ' ₺';
    $db->log('kasa_hareketleri1', (int) $insertID, 'EKLEME', $logAciklama);
} catch (Throwable $e) {
