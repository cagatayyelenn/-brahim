...
function ngls_get_pdo($db) {
    if (isset($db->pdo) && $db->pdo instanceof PDO) return $db->pdo;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['gorusme_kaydet'])) {
    ...
    $hatalar = [];
    if ($ad_soyad === '') $hatalar[] = 'Ad Soyad zorunlu.';
    if ($dil === '')      $hatalar[] = 'Dil zorunlu.';
    if ($sonuc === '')    $hatalar[] = 'Sonuç zorunlu.';
    ...
    if (empty($hatalar)) {
        ...
        $_SESSION['gorusme_eski'] = [
            'ad_soyad'=>$ad_soyad,'dil'=>$dil,'aciklama'=>$aciklama,'sonuc'=>$sonuc
        ];
        header("Location: ".$_SERVER['REQUEST_URI']); exit;
    }
}
        ...
        <?php if (!empty($_SESSION['gorusme_hatalar'])): ?>
          <div class="alert alert-danger">
            <ul class="mb-0">
              ...
              <?php endforeach; unset($_SESSION['gorusme_hatalar']); ?>
            </ul>
          </div>
        <?php endif; ?>
          ...
          <table class="table table-striped table-hover align-middle" id="gorusme-tablo">
            ...
            <tbody>
                  ...
                  <form method="post" class="p-2">
                    <div class="row g-2 align-items-end">
                      ...
                      <div class="col-12 col-md-2">
                        <label class="form-label mb-1">Dil <span class="text-danger">*</span></label>
                      ...
                      <div class="col-12 col-md-2">
                        <label class="form-label mb-1">Sonuç <span class="text-danger">*</span></label>
                      ...
                      <div class="col-12 col-md-2 d-grid">
                        <button type="submit" name="gorusme_kaydet" class="btn btn-primary">Ekle</button>
                      </div>
                    </div>
                  </form>
              ...
              <?php if (!empty($gorusmeler)): ?>
                <?php foreach ($gorusmeler as $row): ?>
                  <?php
                    ...
                    if ($s === 'iletişime geçildi' || $s === 'iletisime gecildi') {      // YEŞİL + BEYAZ
                        $rowClass = 'row-green';  $textClass = 'text-white';
                    } elseif ($s === 'kaydedildi') {                                     // MAVİ + BEYAZ
                        $rowClass = 'row-blue';   $textClass = 'text-white';
                    } elseif ($s === 'randevu verildi') {                                // SARI + KOYU
                        $rowClass = 'row-yellow'; $textClass = 'text-dark';
                    } elseif ($s === 'olumsuz') {                                        // KIRMIZI + BEYAZ
                        $rowClass = 'row-red';    $textClass = 'text-white';
                  ...
                  <tr class="<?= $rowClass ?> <?= $textClass ?>">
                    ...
                    <td>
                      <span class="badge bg-dark bg-opacity-50 <?= $textClass==='text-dark' ? 'text-dark' : 'text-white' ?>">
                    ...
                    <td>
                      ...
                      <form method="post" class="d-inline" onsubmit="return confirm('Bu kaydı silmek istiyor musunuz?');">
                ...
                <?php endforeach; ?>
              <?php else: ?>
                <tr><td colspan="8" class="text-muted">Henüz kayıt yok.</td></tr>
              <?php endif; ?>
...
<style>
  #gorusme-tablo td, #gorusme-tablo th { vertical-align: middle; }
  #gorusme-tablo th { font-weight: 600; }

  /* Satır renklerini hücre seviyesinde uygula (DataTables/striped/hover override) */
  .row-green  > td, .row-green  > th { background-color:#198754 !important; color:#fff !important; }
  .row-blue   > td, .row-blue   > th { background-color:#0d6efd !important; color:#fff !important; }
  .row-yellow > td, .row-yellow > th { background-color:#ffc107 !important; color:#212529 !important; }
  .row-red    > td, .row-red    > th { background-color:#dc3545 !important; color:#fff !important; }

  .table-hover > tbody > tr.row-green:hover  > * { background-color:#198754 !important; color:#fff !important; }
  .table-hover > tbody > tr.row-blue:hover   > * { background-color:#0d6efd !important; color:#fff !important; }
  .table-hover > tbody > tr.row-yellow:hover > * { background-color:#ffc107 !important; color:#212529 !important; }
  .table-hover > tbody > tr.row-red:hover    > * { background-color:#dc3545 !important; color:#fff !important; }

  /* Sarı satırdaki link/badge koyu kalsın */
  .row-yellow a, .row-yellow .badge { color:#212529 !important; }
  .row-green  a, .row-green  .badge,
  .row-blue   a, .row-blue   .badge,
  .row-red    a, .row-red    .badge  { color:#fff !important; }

  /* Sütun başlıklarındaki filtre input’ları için ufak düzen */
  #gorusme-tablo thead tr.filters th { padding: .5rem; }
  #gorusme-tablo thead tr.filters input { height: 32px; font-size: .85rem; }
</style>
							...
							<div class="card-body">
								<div class="list-tab mb-4">
									<ul class="nav">
										...
										<li>
											<a href="index.html#" data-bs-toggle="tab" data-bs-target="#teachers">Öğretmenler</a>
										</li>
										<li>
								...
								<div class="d-md-flex align-items-center justify-content-between">
									<div class="me-md-3 mb-3 mb-md-0 w-100">
										...
										<div
											...
											<p class="mb-0 me-2"><i
													class="ti ti-arrow-badge-down-filled me-2 text-danger"></i>Ortalamanın altında
											</p>
											<h5>02</h5>
										</div>
									</div>
					...
					<div class="col-xxl-8 col-xl-6 d-flex">
						<div class="card flex-fill">
							<div class="card-header  d-flex align-items-center justify-content-between">
								...
								<div class="dropdown">
									<a href="javascript:void(0);" class="bg-white dropdown-toggle"
										data-bs-toggle="dropdown"><i class="ti ti-calendar  me-2"></i>Son 1 Hafta
									</a>
									<ul class="dropdown-menu mt-2 p-3">
										<li>
											<a href="javascript:void(0);" class="dropdown-item rounded-1">
											...
											<div class="d-flex align-items-center overflow-hidden me-2">
												...
												<div class="overflow-hidden">
													<h6 class="mb-1 text-truncate"><a
															href="javascript:void(0);">Timoor</a><span
									...
									<div class="card-body p-3">
										<div class="d-flex align-items-center justify-content-between mb-3">
											<div class="d-flex align-items-center overflow-hidden me-2">
												...
												<div class="overflow-hidden">
													<h6 class="mb-1 text-truncate "><a
															href="javascript:void(0);">Ali</a><span
					...
					<div class="col-xxl-4 col-xl-6 d-flex">
						<div class="card flex-fill">
							<div class="card-header  d-flex align-items-center justify-content-between">
								...
								<div class="dropdown">
									<a href="javascript:void(0);" class="bg-white dropdown-toggle"
										data-bs-toggle="dropdown"><i class="ti ti-school-bell  me-2"></i>Sınıf
									</a>
									<ul class="dropdown-menu mt-2 p-3">
										<li>
											<a href="javascript:void(0);" class="dropdown-item rounded-1">
					...
					<div class="col-xxl-4 col-xl-6 d-flex">
						<div class="card flex-fill">
							<div class="card-header  d-flex align-items-center justify-content-between">
								...
								<div class="dropdown">
									<a href="javascript:void(0);" class="bg-white dropdown-toggle"
										data-bs-toggle="dropdown"><i class="ti ti-calendar me-2"></i>Bu Hafta
									</a>
									<ul class="dropdown-menu mt-2 p-3">
										<li>
											<a href="javascript:void(0);" class="dropdown-item rounded-1">
					...
					<div class="col-xxl-4 col-xl-12 d-flex">
						<div class="card flex-fill">
							<div class="card-header  d-flex align-items-center justify-content-between">
								...
								<div class="dropdown">
									<a href="javascript:void(0);" class="bg-white dropdown-toggle"
										data-bs-toggle="dropdown"><i class="ti ti-calendar me-2"></i>Bugün
									</a>
									<ul class="dropdown-menu mt-2 p-3">
										<li>
											<a href="javascript:void(0);" class="dropdown-item rounded-1">
